...
In this section, we apply ASURAT on human cancer transcriptomes.
...
**Tips:**
Disease Ontology DB is only for human disease.
...
Input `tidy_DO` into the ASURAT objects.
```{r, eval = FALSE}
...
sc68_cisp <- do_quickQC_sign(obj = sc68_cisp, data_type = "DO",
                             min_ngenes = 2, max_ngenes = 1000)
```
The computational results are stored in `obj[["sign"]][["DO"]][["disease"]]`.
...
```
The computational result is stored in `obj[["sign"]][["DO"]][["disease"]]`.
...
The arguments are `obj`, `data_type`, `measure` (argument of `doSim()` in `DOSE`
package: one of `"Wang"`, `"Resnik"`, `"Rel"`, `"Jiang"`, and `"Lin"`),
`orgdb = NULL`, `treeTable = NULL`, and `IC = IC_DO`.
...
`obj[["sign"]][["DO_Sim"]][["disease"]][["report"]]`.
...
principal component used for t-SNE, in which `NULL` is accepted if users would
like to compute t-SNE from the original sample-by-sign matrices), and
`tsne_dim` (t-SNE dimension: either `2` or `3`).
```{r, eval = FALSE}
...
`title`, `title_size`, `xlabel`, `ylabel`, and `default_color` (if `TRUE`,
...
**Tips:**
Since diffusion maps are strongly affected by outliers, the eigenvectors for
the dimension reduction should be properly chosen.
```{r, eval = FALSE}
...
sc68_cisp <- classify_samples_merlot_sign(
  obj = sc68_cisp, data_type = "DO", category = "disease")
```
The computational result is stored in `obj[["sample"]][["merlot_DO_disease"]]`.
...
`plot_dmap_geneExpression()` show the normalized gene expression levels for
the input gene on the low-dimensional space.
```{r, eval = FALSE}
# ----------------------------------------
# For `day1_norm`
# ----------------------------------------
...
Load the objects.
```{r, eval = FALSE}
day1_norm <- readRDS(file = "backup/01_104_day1_norm_DES_DO.rds")
day7_hypo <- readRDS(file = "backup/02_104_day7_hypo_DES_DO.rds")
sc68_vehi <- readRDS(file = "backup/03_104_sc68_vehi_DES_DO.rds")
sc68_cisp <- readRDS(file = "backup/04_104_sc68_cisp_DES_DO.rds")
```
