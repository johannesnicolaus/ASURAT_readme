...
In this section, we apply ASURAT on healthy donor-derived single-cell
transcriptomes.
...
**Tips:**
Cell Ontology database is basically used for analyses of human disease
sample.
...
Input a formatted Cell Ontology database `tidy_CO` into the ASURAT object.
```{r, eval = FALSE}
...
between user's data and `tidy_CO`, and then excludes the IDs including too
...
pbmc_6000 <- do_quickQC_sign(obj = pbmc_6000, data_type = "CO",
                             min_ngenes = 2, max_ngenes = 1000)
```
The results are stored in `obj[["sign"]][["CO"]][["cell"]]`.
...
```
The result is stored in `obj[["sign"]][["CO"]][["cell"]]`.
...
pbmc_6000_red <- compute_SemSim_sign(obj = pbmc_6000_red, data_type = "CO",
                                     ...
                                     treeTable = treeTable_CO, IC = IC_CO)
```
The result is stored in `obj[["sign"]][["CO_Sim"]][["cell"]]`.
...
pbmc_6000_red <- reduce_sign(obj = pbmc_6000_red, data_type = "CO",
                             threshold = 0.95, keep_rareID = TRUE)
```
The result is stored in `obj[["sign"]][["CO_Sim"]][["cell"]][["report"]]`.
...
pbmc_6000_red <- make_signxsample_matrix(obj = pbmc_6000_red, data_type = "CO",
                                         weight_strg = 0.5, weight_vari = 0.5)
```
The result is stored in `obj[["sign"]][["COxSample"]][["cell"]]`.
...
```
The result is stored in `obj[["reduction"]][["pca"]][["CO"]][["cell"]]`.
...
pbmc_6000 <- do_tsne_sign(obj = pbmc_6000, data_type = "CO", category = "cell",
                          pca_dim = NULL, tsne_dim = 2)
```
The result is stored in `obj[["reduction"]][["tsne"]][["CO"]][["cell"]]`.
...
pbmc_6000 <- do_umap_sign(obj = pbmc_6000, data_type = "CO",
                          category = "cell", pca_dim = NULL, umap_dim = 2)
```
The result is stored in `obj[["reduction"]][["umap"]][["CO"]][["cell"]]`.
...
pbmc_6000 <- do_DiffusionMap_sign(obj = pbmc_6000, data_type = "CO",
                                  category = "cell", pca_dim = NULL)
```
The result is stored in `obj[["reduction"]][["dmap"]][["CO"]][["cell"]]`.
...
```
The results are stored in `obj[["classification"]][["pam"]][["CO"]][["cell"]]`
and `obj[["sample"]][["pam_CO_cell"]]`.
