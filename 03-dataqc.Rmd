...
In this section, we remove the genes and samples with suspicious qualities,
by processing the following three steps:
(1) quick gene filtering, (2) sample filtering, and
(3) second gene filtering.
...
The following function `do_quickQC()` removes the variables for which the number
of non-zero expressing samples is less than `min_nsamples`.
...
input meta data for each sample.
...
The arguments are `vec` (one-dimensional vector),
...
Note that the annotation value is an approximate one.
```{r, eval = FALSE}
...
The following function `trim_samples()` filters out the suspicious quality
samples by setting minimum and maximum values as `min_foo` and `max_foo`,
respectively.
```{r, eval = FALSE}
...
The following function `trim_variables()` filters out the suspicious quality
...
Check the scatter plots between the meta data for each sample.
```{r, eval = FALSE}
my_func <- function(obj, metaname1, metaname2){
  df <- data.frame(x = obj[["sample"]][[metaname1]],
                   y = obj[["sample"]][[metaname2]])
