...
Input `tidy_GO` into the ASURAT objects.
```{r, eval = FALSE}
...
between user's data and `tidy_GO`, and then excludes the IDs including too few
...
The arguments are `obj`, `data_type`, `measure` (argument of `GOSemSim()`
function `mgoSim`: one of `"Resnik"`, `"Rel"`, `"Jiang"`, and  `"Lin"`),
`orgdb` (annotation data package such as `org.Hs.eg.db` and `org.Mm.eg.db`),
`treeTable = NULL`, and `IC = IC_GO`.
...
`obj[["classification"]][["pam"]][["GO"]][["BP"]]` and
...
```
The computational results are stored in
`obj[["sample"]][["seuratFindClusters_GO_BP"]]` and
...
```
```
Loaded Tree Topology Library...
Data Dimensions (2285, 3)
Calculating distances...
Calculating Scaffold Tree...
Endpoints: 3 3 1888 1012
Branchpoints: 1 1138
Tree_Branch 3 1138
Tree_Branch 1012 1138
Tree_Branch 1888 1138
Finished Scaffold Tree... 111.95978808403015 seconds
```
The computational result is stored in
...
sc68_cisp <- classify_samples_merlot_sign(
  obj = sc68_cisp, data_type = "GO", category = "BP")
```
The computational result is stored in `obj[["sample"]][["merlot_GO_BP"]]`.
